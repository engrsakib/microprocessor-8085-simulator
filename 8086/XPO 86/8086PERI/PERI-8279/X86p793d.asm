...
;In the left entry mode the characters are entered like the characters  on the
;typewriter. The first character is written to the leftmost display  the next 
;character entered is displayed to the right of the first one and  this 
;continues for next characters. The 17th character is again displayed  on the  
;leftmost display because it is a 16 character left entry mode.

X86P795D  SEGMENT

	ASSUME CS:X86P795D, DS:X86P795D ,ES:X86P795D
...
START:	
	MOV  AL,08H	; C/W for 8-8 bit 
	MOV  DX,01E2H   ; character right entry.
	OUT  DX,AL    
	MOV  AL,0DFH
	...
	OUT  DX,AL	; mode to write
			; Display RAM.
	MOV  CL,0FH	; Set counter for look up 
			; table.
